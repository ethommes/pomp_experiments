...
## ----sir_sim1------------------------------------------------------------
measSIR %>%
  simulate(params=c(Beta=7.5,mu_IR=0.5,rho=0.5,eta=0.03,N=1e8),
...
#' 
#' New - Ed:
#' Let's see if we can cook up something closer to flu:
#' (Note that time is in units of weeks!)
...
fluSIR %>%
  simulate(params=c(Beta=R_0*gamma_recov,mu_IR=gamma_recov,rho=1,eta=(1-R_0_factor),N=Npop),
           nsim=1,format="data.frame",include.data=TRUE) -> sims_flu
